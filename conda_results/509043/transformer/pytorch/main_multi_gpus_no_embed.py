...
from .utils.eval_multi_gpu_no_embed import evaluate
from .utils.param_count import count_parameters
from .utils.train_multi_gpu_no_embed import train
...
def main(
    ...
    if sample_size:
        ...
        # ].tolist()
        train_indices = np.random.choice(len(train_labels), size=size, replace=False)
        ...
        test_indices = np.random.choice(len(test_labels), size=size, replace=False)
    ...
    for iepoch in range(n_epochs):
        with tqdm(
            ...
            val_loss, val_acc, val_auc = evaluate(
                model, val_loader, criterion, max_seq_len, accelerator
            )
