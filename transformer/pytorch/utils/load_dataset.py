...
def extract_batch_number(file_path):
    match = re.search(r"_batch_(\d+).pt", file_path)
    if match:
        return int(match.group(1))
    else:
        return -1  # Or raise an error if the pattern doesn't match
...
def get_dataset(
    ...
    tensor_list = [
        load_tensor(f)
        for f in tqdm(
            sorted(file_list, key=extract_batch_number),
            desc=f"Loading {type} tensors",
        )
    ]
    combined_tensor = torch.cat(tensor_list, dim=0)
    with open(
        f"{group_dir}/QuantumTransformer/data/word_labels/{name}_{type}_labels.pkl",
        "rb",
    ) as f:
...
def load_dataset(name: Literal["amazon", "imdb", "yelp"]):
    ...
    if len(train_labels) > size_limit:
        sampled_train_labels, sampled_train_data = sampling(
            train_labels, train_data, size_limit
        )
